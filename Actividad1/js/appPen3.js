...
d3.csv('credPen.csv').then((data) => {
  ...
  data = data.filter((d) => {
    return (d.CREDITO_PENSIONADOS > 0)
  })
  // data = data.filter((d) => (d.ANIO > 0) && (d.CREDITO_PENSIONADOS > 0))
  ...
  y.domain([d3.min(data, d => d.CREDITO_PENSIONADOS),
            d3.max(data, d => d.CREDITO_PENSIONADOS)])
...
function render(data) {
  ...
  p.enter()
    .append('circle')
      .attr('r', 0)
      .attr('cx', d => x(d.ANIO))
      .attr('cy', d => y(d.CREDITO_PENSIONADOS))
      .attr('fill', '#005500')
